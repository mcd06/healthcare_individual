...
if st.session_state.authenticated:
    ...
    measure = st.sidebar.selectbox("Select Measure", ["Incidence", "Deaths"])
    metric = st.sidebar.radio("Select Metric", ["Number", "Rate"])
    ...
    if filtered_df.empty:
        ...
        if metric == "Rate":
            total_val = filtered_df["val"].mean()
            male_val = latest_df[latest_df["gender"] == "Male"]["val"].mean()
            female_val = latest_df[latest_df["gender"] == "Female"]["val"].mean()
            display_label = "Avg"
        else:
        ...
        k1.metric(f"{display_label} {label_y}", f"{total_val:,.1f}")
        k2.metric(f"Latest Male {metric}", f"{male_val:,.1f}")
        k3.metric(f"Latest Female {metric}", f"{female_val:,.1f}")
        ...
        fig_heat.update_layout(title="Cancer Burden by Age and Year", title_x=0.0, height=260, margin=dict(t=50, b=10))
        ...
        fig_pie.update_layout(height=260, title_x=0.0)
        ...
        fig_scatter = px.scatter(
            ...
            color="gender",
            color_discrete_map=gender_colors,
            title=f"Gender-wise {metric} per Year",
        ...
        fig_scatter.update_layout(height=260, title_x=0.0, plot_bgcolor='white', xaxis=dict(showgrid=False), yaxis=dict(showgrid=False), hovermode="closest")
